...
class LabelEmbedder(nn.Module):
    ...
    def forward(self, conditions, train, force_drop_ids=None):
        ...
        conditions = conditions.to(device=self.linear.weight.device, dtype=self.linear.weight.dtype)
...
#                      Embedding Layers for Actions and                         #
#################################################################################
class PerTokenEmbedder(nn.Module):
    def __init__(self, per_token_size, hidden_size):
        super().__init__()
        self.linear = nn.Linear(per_token_size, hidden_size)
...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        # Initialize per-token embedder:
        if self.use_per_attn:
            nn.init.normal_(self.per_token_embedder.linear.weight, std=0.02)
            nn.init.constant_(self.per_token_embedder.linear.bias, 0)
    ...
    def forward(self, x, t, z, per_token=None):
        ...
        x = x + self.positional_embedding[: x.shape[1]]     # (N, T+1, D)
